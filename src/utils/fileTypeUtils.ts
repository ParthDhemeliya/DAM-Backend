import { FileType, FILE_TYPE_MAPPING, FILE_SIZE_LIMITS, ALLOWED_EXTENSIONS } from '../interfaces/asset.interface'
 ...
 */
export function validateFileExtension(filename: string, fileType: FileType): boolean {
  const ext = path.extname(filename).toLowerCase().slice(1)
...
export function generateStoragePath(
  ...
  // Organize by type and date: assets/images/2024/01/13/filename.jpg
...
export function validateFileForUpload(
  ...
  if (!validateFileSize(fileSize, fileType)) {
    ...
    errors.push(`File size exceeds limit for ${fileType} files. Max: ${maxSize}`)
  }
